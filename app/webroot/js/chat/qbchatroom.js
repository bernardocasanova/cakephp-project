...
function autoLogin() {
	if (localStorage['QBChatUser']) {
        ...
        $('.loading_messages').addClass('hidden');
    }
}
		...
		if (trim(post)) {
			...
			if (userJID) {
				...
				showMessage(qbID,
				            post,
				            chatUser.name,
				            chatUser.avatar || 'js/chat/images/avatar_default.jpg',
				            chatUser.fbID || '',
				            null,
				            null,
...
function getMessage(stanza, room) {
	...
	var qbID, message, name, avatar, fbID, icon, defaultAvatar = 'js/chat/images/avatar_default.jpg';
	var chatID, selector;
...
function showMessage(qbID, message, name, avatar, fbID, icon, createTime, messageTime, selector) {
    ...
    html = '<section class="message show-actions clearfix" data-qb="' + qbID + '" data-fb="' + fbID + '">';
        //html += '<img class="message-icon" src="' + icon + '" alt="icon">';
        html += '<img class="message-avatar" src="' + avatar + '" alt="avatar">';
        html += '<div class="message-body">';
            html += '<footer><b class="message-author">' + name + '</b>';
            html += '<span class="message-description">' + parser(message, createTime) + '</span>';
//            html += '<time class="message-time" datetime="' + messageTime + '">' + $.timeago(messageTime) + '</time>';
            html += '</footer>';
        html += '</div>';
	html += '</section>';
...
function showComposing(composing, qbID, type) {
	...
	if (composing && qbID != chatUser.qbID) {
		if (selector.find('span').is('.typing'))
			...
			selector.find('.chat-content').append('<span class="typing">' + name + ' está digitando...</span>');
